...
class BlochusPulse(basic):
    ...
    def bloch_fcn(self, time, mag):
        ...
        if time <= self.Pulse.t_pulse:
            ...
            # merge both B-fields
            b_total = np.array([bp_amp[0], bp_amp[1], self.B0])
    ...
    def get_pulse_amplitude(self, time):
        ...
        if self.discretization == 'cont':
            ...
            if self.polarization == 'circ':
                bp_amp = current*pulse_amp*np.array([cos_term, sin_term])
            else:
                bp_amp = 2.0*current*pulse_amp*np.array([cos_term, 0.0])
        else:
            bp_amp = np.array([0.0, 0.0])
        return bp_amp
